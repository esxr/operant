...
import { getMode, getOperantApiKey, getOperantApiUrl, getWhatsAppParticipants, getConversationSid, writeConversationSid, } from './config.js';
function getConfig() {
 ...
 */
function callConversationsApi(accountSid, authToken, method, path, body, httpClient = https) {
    return new Promise((resolve, reject) => {
 ...
 */
async function ensureConversation(accountSid, authToken, whatsappNumber, participants, getSid = getConversationSid, writeSid = writeConversationSid, httpClient = https) {
    // Step 1: Check for a cached SID
    let sid = getSid();
    if (sid) {
        // Step 2: Validate the stored SID
        const checkResult = await callConversationsApi(accountSid, authToken, "GET", `/v1/Conversations/${sid}`, undefined, httpClient);
        ...
        if (checkResult.status === 404) {
            ...
            throw new Error(`Conversations API error ${checkResult.status} validating SID ${sid}: ` +
    ...
    if (!messagingServiceSid) {
        process.stderr.write("[whatsapp] TWILIO_MESSAGING_SERVICE_SID not set; Conversation creation may fail outside sandbox.\n");
    }
    else {
        createBody.MessagingServiceSid = messagingServiceSid;
    }
    const createResult = await callConversationsApi(accountSid, authToken, "POST", "/v1/Conversations", createBody, httpClient);
    if (createResult.status < 200 || createResult.status >= 300) {
        throw new Error(`Conversations API error ${createResult.status} creating conversation: ` +
            JSON.stringify(createResult.data).slice(0, 200));
    }
    const newSid = createResult.data.sid;
    ...
    for (const number of participants) {
        ...
        const addResult = await callConversationsApi(accountSid, authToken, "POST", `/v1/Conversations/${newSid}/Participants`, participantBody, httpClient);
        if (addResult.status === 409) {
            process.stderr.write(`[whatsapp] Participant ${number} already in conversation, skipping\n`);
        }
        else if (addResult.status < 200 || addResult.status >= 300) {
            process.stderr.write(`[whatsapp] Failed to add participant ${number} to conversation ${newSid}: ` +
 ...
 * @param httpClient      Injectable https module for unit tests
 */
async function sendConversationMessage(accountSid, authToken, conversationSid, body, httpClient = https) {
    const result = await callConversationsApi(accountSid, authToken, "POST", `/v1/Conversations/${conversationSid}/Messages`, { Body: body, Author: "operant" }, httpClient);
    if (result.status < 200 || result.status >= 300) {
        throw new Error(`Conversations API error ${result.status} sending message: ` +
    ...
    try {
        ...
        if (participants.length > 0) {
            ...
            if (!accountSid || !authToken || !whatsappNumber) {
                process.stderr.write("[whatsapp] Missing Twilio config for Conversations API, falling back to 1:1\n");
                // fall through to 1:1 path below
            }
            else {
                try {
                    const sid = await ensureConversation(accountSid, authToken, whatsappNumber, participants);
                    await sendConversationMessage(accountSid, authToken, sid, message);
                    process.stderr.write(`[whatsapp] Sent to conversation ${sid} (${participants.length} participants)\n`);
                    return;
                }
                catch (err) {
                    process.stderr.write(`[whatsapp] Conversations API error: ${err.message}, falling back to 1:1\n`);
                    // fall through to 1:1 path below
                }
            }
        }
        ...
        process.stderr.write(`[whatsapp] Notification failed: ${err.message}\n`);
    }
}
// ---------------------------------------------------------------------------
// WhatsAppChannel
        ...
        if (participants.length > 0) {
            ...
            if (accountSid && authToken && whatsappNumber) {
                try {
                    ...
                    this.log(`[whatsapp] Conversations API error: ${err.message}, falling back to 1:1`);
