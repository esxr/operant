    ...
    ["sdlc_review", new Map([
            ["REVIEW_APPROVED", {
                    to: (ctx) => {
                        ...
                        const reviewed = ctx.reviewedArtifact;
                        if (!reviewed || !SDLC_ORDER.includes(reviewed))
                            return "dev";
                        return nextSdlcState(reviewed);
                    },
                    sideEffects: (ctx) => {
                        const reviewed = ctx.reviewedArtifact;
                        const next = reviewed && SDLC_ORDER.includes(reviewed)
                            ? nextSdlcState(reviewed)
                            : "dev";
                        const specDir = ctx.specDir ?? "";
                        if (next === "dev") {
 ...
 */
export function stateToPhase(state) {
    switch (state) {
        case "idle":
        case "call_active":
        case "complete":
 ...
 */
export function transition(current, event, context) {
 ...
 */
export function getCurrentSpec(specsRoot) {
 ...
 */
export function listSpecs(specsRoot) {
    if (!existsSync(specsRoot))
        return [];
    const results = [];
    const entries = safeReaddir(specsRoot);
 ...
 */
export function detectNewBlockers(specDir, knownBlockers) {
 ...
 */
export function detectNewRevisions(specDir, knownRevisions) {
 ...
 */
export function classifyTranscript(transcript, callAnalysis) {
    ...
    if (callAnalysis && typeof callAnalysis === "object") {
        ...
        if (customData && typeof customData === "object") {
            const nestedSummary = customData.call_summary;
            ...
            const featureName = customData.feature_name;
    ...
    if (doneStandalone.test(lower))
        confirmCount++;
    if (yesStandalone.test(lower))
        confirmCount++;
 ...
 */
export function deriveSpecName(featureName) {
 ...
 */
export function inferPhase(specsRoot) {
 ...
 */
export function inferState(specsRoot) {
    if (!existsSync(specsRoot))
        return "idle";
    // Collect spec directories (excluding .operant/)
    const specDirs = safeReaddir(specsRoot).filter((entry) => entry !== ".operant" && isDirectory(join(specsRoot, entry)));
    ...
    for (const dir of specDirs) {
        ...
        if (hasBlockersDir && hasEis) {
            const blockerFiles = safeReaddir(blockersDir).filter((f) => f.endsWith(".md"));
        ...
        if (hasRevisionsDir && hasEis) {
            const revisionFiles = safeReaddir(revisionsDir).filter((f) => f.endsWith(".md"));
            if (revisionFiles.length > 0) {
                ...
                if (!hasOpenBlockers) {
                    ...
                    const newestRevisionMtime = Math.max(...revisionFiles.map((f) => safeStatMtime(join(revisionsDir, f))));
 ...
 */
function isSpecComplete(reqPath) {
 ...
 */
function detectNewFiles(dirPath, knownFiles) {
    if (!existsSync(dirPath))
        return [];
