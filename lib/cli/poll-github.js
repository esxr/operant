...
import { getDataDir, ensureDataDir, getGitHubRepo, getGitHubToken, getGitHubPollInterval, } from "../config.js";
// ---------------------------------------------------------------------------
// Constants
// ---------------------------------------------------------------------------
...
function fetchIssues(owner, repo, token) {
    return new Promise((resolve, reject) => {
        ...
        const req = https.request({
            ...
            res.on("end", () => {
                const rateLimitRemaining = parseInt(res.headers["x-ratelimit-remaining"] ?? "5000", 10);
...
// Idempotency Check
// ---------------------------------------------------------------------------
function triggerExists(dataDir, issueNumber) {
...
// Trigger File Writer
// ---------------------------------------------------------------------------
function writeTriggerFile(issue, dataDir) {
    const pendingDir = join(dataDir, "pending");
...
async function pollOnce() {
    ...
    try {
        ...
        if (rateLimitRemaining < 100) {
            process.stderr.write(`${LOG_PREFIX} WARNING: GitHub API rate limit low (${rateLimitRemaining} remaining)\n`);
        }
        for (const issue of issues) {
            if (issue.number <= cursor)
                continue;
        ...
        process.stderr.write(`${LOG_PREFIX} Error: ${err.message}\n`);
    }
    process.stderr.write(`${LOG_PREFIX} Checked at ${new Date().toISOString()}. Found ${newCount} new issue(s).\n`);
    return newCount;
}
// ---------------------------------------------------------------------------
// Main
