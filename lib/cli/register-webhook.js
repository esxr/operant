...
// Helpers
// ---------------------------------------------------------------------------
function loadTunnelUrl(dataDir) {
...
async function main() {
    ...
    if (!tunnelUrl) {
        process.stderr.write("Usage: node lib/cli/register-webhook.js [tunnel-url]\n" +
            "\n" +
    ...
    try {
        ...
        process.stderr.write(`Failed to get agent ID: ${err.message}\n`);
        process.exit(1);
        return;
    }
    ...
    try {
        ...
        process.stderr.write(`Failed to register webhook: ${err.message}\n`);
        process.stdout.write(JSON.stringify({
            success: false,
            agentId,
            webhookUrl,
