 ...
 */
import { readFileSync, readdirSync, renameSync, mkdirSync, writeFileSync, unlinkSync } from "node:fs";
import { join, basename } from "node:path";
import { transition, InvalidTransitionError, } from "../state-machine.js";
import { getDataDir, getSpecsRoot, readState, writeState, readActiveSpec, ensureDataDir, } from "../config.js";
import { classifyComplexity, getTimeout, } from "../channel.js";
...
function loadWhitelistTarget(dataDir) {
 ...
 */
async function sendVoiceCall(context) {
 ...
 * or null on timeout.
 */
async function pollForReply(mode, timeoutMs) {
    const dataDir = getDataDir();
    ...
    return new Promise((resolve) => {
        const timer = setInterval(() => {
            ...
            for (const file of files) {
                ...
                const replyText = (payload.body ?? payload.raw_transcript ?? payload.reply ?? "");
                const parsed = parseReply(replyText);
                clearInterval(timer);
                resolve({
                    interactionId: (payload.call_id ?? payload.message_sid ?? file),
...
// ---------------------------------------------------------------------------
function buildReviewContext(specDir, specName, artifactType) {
...
function buildBlockerContext(specDir, specName) {
...
function buildConfirmationContext(specDir, specName) {
...
async function main() {
    ...
    if (filteredArgs.length < 1) {
        process.stderr.write("Usage: node lib/cli/trigger-gate.js [--poll-only] <mode> [artifactType] [specDir]\n" +
            "\n" +
    ...
    const channel = complexity === "complex" ? "voice" : "whatsapp";
    const timeoutMs = getTimeout(mode);
    const result = {
        mode,
        channel,
        reply: null,
    ...
    if (pollOnly) {
        ...
        try {
            ...
            if (channel === "whatsapp") {
                process.stderr.write(`[trigger-gate] WhatsApp failed: ${err.message}, escalating to voice\n`);
                try {
                    await sendVoiceCall(context);
                    ...
                    process.stderr.write(`[trigger-gate] Voice also failed: ${voiceErr.message}\n`);
                    process.exit(1);
                }
            }
            else {
                process.stderr.write(`[trigger-gate] Voice call failed: ${err.message}\n`);
                process.exit(1);
            }
        }
    }
    ...
    if (!reply) {
        ...
        if (!isMock && channel === "whatsapp") {
            ...
            try {
                ...
                const voiceTimeout = getTimeout("blocker"); // voice gets full timeout
                ...
                result.timedOut = true;
                process.stderr.write(`[trigger-gate] Voice escalation failed: ${err.message}\n`);
            }
        }
        else {
            result.timedOut = true;
...
main().catch((err) => {
    process.stderr.write(`[trigger-gate] Fatal: ${err.message}\n`);
    process.exit(1);
});
