...
function moveToProcessed(triggerPath: string): string {
...
function main(): void {
  ...
  try {
    ...
    process.stderr.write(`Failed to read trigger file: ${(err as Error).message}\n`);
    process.exit(1);
  ...
  const featureName = payload.spec?.feature_name
    ?? (callAnalysis?.custom_analysis_data as Record<string, unknown> | undefined)?.feature_name as string | undefined
    ?? "";
