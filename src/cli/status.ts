...
function loadWhitelist(dataDir: string): Whitelist {
...
function checkPid(pidFile: string, dataDir: string): string {
  const pidPath = join(dataDir, pidFile);
  if (!existsSync(pidPath)) return "not running";
  try {
...
function readTunnelUrl(dataDir: string): string {
...
function main(): void {
  ...
  if (specs.length > 0) {
    ...
    for (const spec of specs) {
      ...
      const artifacts = [];
      if (spec.artifacts.intent) artifacts.push("intent");
      if (spec.artifacts.hld) artifacts.push("hld");
      if (spec.artifacts.adr) artifacts.push("adr");
      if (spec.artifacts.eis) artifacts.push("eis");
